...
Socket::Socket(IpVer ipver) : ipver_(ipver)
{
  ...
  Verbose::show("Socket(IpVer ipver) constructor is called \n");
}
...
Socket::Socket(::SOCKET sock) : socket_(sock)
{
  ...
  hints.ai_socktype = SOCK_STREAM;
  hints.ai_protocol = IPPROTO_TCP;
  Verbose::show("Socket(::SOCKET sock) constructor is called \n");
}
...
Socket::Socket(Socket&& s)
{
  ...
  Verbose::show("The socket move constructor is called \n");
}
...
Socket& Socket::operator=(Socket&& s)
{
  ...
  hints.ai_socktype = s.hints.ai_socktype;
  hints.ai_protocol = s.hints.ai_protocol;
  Verbose::show("The socket move assignment operator is called \n", always);
  return *this;
}
//----< get, set IP version >------------------------------------------------
/*
...
Socket::~Socket() {
  Verbose::show("Socket destructor is called \n");
  shutDown();
  close();
}
//----< send buffer >--------------------------------------------------------
...
{
  hints.ai_family = AF_UNSPEC;
  Verbose::show("Sender Socket Connector is being created \n", always);
}
...
SocketConnecter::SocketConnecter(SocketConnecter&& s) : Socket()
{
  ...
  hints.ai_socktype = s.hints.ai_socktype;
  hints.ai_protocol = s.hints.ai_protocol;
  Verbose::show("Sender Socket Connector is being moved \n");
}
...
SocketConnecter& SocketConnecter::operator=(SocketConnecter&& s)
{
  ...
  hints.ai_socktype = s.hints.ai_socktype;
  hints.ai_protocol = s.hints.ai_protocol;
  Verbose::show("Sender Socket Connector is being move assigned \n");
  return *this;
}
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
  ...
  for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
    ...
    if (iResult == SOCKET_ERROR) {
      ...
      int error = WSAGetLastError();
      Verbose::show("WSAGetLastError returned " + toString(error), always);
      continue;
    }
    break;
  }
...
SocketListener::SocketListener(size_t port, IpVer ipv) : Socket(ipv), port_(port)
{
  ...
  Verbose::show("Receiver Listen Socket is being created \n", always);
}
//----< move constructor transfers ownership of Win32 socket_ member >-------
...
SocketListener::SocketListener(SocketListener&& s) : Socket()
{
  ...
  Verbose::show("Receiver Listen Socket is being moved \n");
}
//----< move assignment transfers ownership of Win32 socket_ member >--------
...
SocketListener& SocketListener::operator=(SocketListener&& s)
{
  ...
  hints.ai_protocol = s.hints.ai_protocol;
  hints.ai_flags = s.hints.ai_flags;
  Verbose::show("Receiver Listen Socket is being move assigned \n");
  return *this;
}
...
bool SocketListener::bind()
{
  ...
  if (iResult != 0) {
    Verbose::show("\n  -- getaddrinfo failed with error in bind(): " + toString(iResult), always);
    return false;
  }
...
Socket SocketListener::accept()
{
  ...
  if (!clientSocket.validState()) {
	...
	Verbose::show("Returning clientSocket after accepting failed \n", always);
    return clientSocket;
  }
  Verbose::show("Returning clientSocket after accepting and construction \n", always);
  return clientSocket;
}
...
//-----<The following code commented out by Wang for adaptation to project 3 on Apr 10, 2015
