...
public:
  ...
  bool start(CallObj& co, Receiver& rcv);
  /*template<typename CallObj>
  bool start(CallObj& co, ClientReceiver& rcv);*/
  void stop();
private:
  bool bind();
  bool listen();
...
*/
template<typename CallObj>
bool SocketListener::start(CallObj& co, Receiver& rcv)
{
  if (!bind())
  {
    return false;
  ...
  std::thread ListenThread(
    [&]()
    {
      while (!acceptFailed_)
      {
		...
		Verbose::show("Construct the client Socket in ListenThread from the socket returned by accept() \n", always);
        if (!clientSocket.validState()) {
          continue;
        }
        Verbose::show("-- Server/Client itself accepted connection \n", always);

        // start thread to handle client request
		Verbose::show("Client Socket is being moved \n", always);
		std::thread clientThread(std::ref(co), std::move(clientSocket) , std::ref(rcv));
		
        clientThread.detach();  // detach - listener won't access thread again
      }
      Verbose::show("-- Listen thread of the Server/Client itself stopping", always);
    }
  );
  ListenThread.detach();
  return true;
...
//template<typename CallObj>
//bool SocketListener::start(CallObj& co, ClientReceiver& rcv)
//{
//	if (!bind())
//	{
//		return false;
